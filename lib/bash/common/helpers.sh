...
function installPackage {
  ...
  if [ "0" -eq "${exitcode}" ]; then
    juju-log "Successfully installed package ${package}";
  else
    # a chance to log the error
    juju-log "Error ${exitcode} installing package ${package}";
...
function isPackageInstalled {
  ...
  if dpkg --get-selections | grep -q "^${1}[[:space:]]*install$" >/dev/null; then
    juju-log "Package ${1} is already installed.";
    return 0
  else
    juju-log "Package ${1} is not installed.";
    return 33
  fi
}
...
function purgePackage {
  ...
  if [ "0" -eq  "${exitcode}" ]; then
    juju-log "Purging old ${package} installation at ${JUJU_REMOTE_UNIT:-unset}"
...
function installPuppetModule {
  ...
  if [ "0" -eq  "${exitcode}" ]; then
    juju-log "Successfully installed module ${module}"
  else
    juju-log "Failed to install module ${module}"
    exit ${exitcode}
  fi
}
...
function uninstallPuppetModule {
  ...
  if [ "0" -eq  "${exitcode}" ]; then
    juju-log "Successfully uninstalled module ${module}"
  else
    juju-log "Failed to uninstall module ${module} with exit code ${exitcode}. Module was not installed most likely"
  fi
...
function testPuppetAgent {
  ...
  if [ "0" -eq "${exitcode}" ]; then
    juju-log "Successfully tested puppet agent connecting to ${external_puppetmaster}";
  else
    juju-log "Puppet agent failed to connect to ${external_puppetmaster}";
    exit ${exitcode}
  fi
}
...
function puppetApply {
  ...
  puppet apply --debug --verbose "${DIR}/../../${manifestpath:-lib/puppet/install.pp}"
}
...
function testPuppetApply {
  ...
  if [[ ! -d "$DIR" ]]; then DIR="$PWD"; fi
  local exitcode=0
  # todo: why not ${DIR}/../puppet/notifytest.pp
  puppet apply --test "${DIR}/../lib/puppet/notifytest.pp" || exitcode=$?
  if [ "0" -eq "${exitcode}" ]; then
    juju-log "Successfully tested puppet apply";
  else
    juju-log "Failed to run puppet apply";
    exit ${exitcode}
  fi
}
...
function startUpstartJob {
  ...
  then
    ...
    if [ "0" -eq "${exitcode}" ]
    then
      juju-log "Successfully restarted ${job}";
    else
      juju-log "Failed to restart ${job}"
      exit ${exitcode}
    fi;
  else
    start ${job} || exitcode=$?
    if [ "0" -eq "${exitcode}" ]
    then
      juju-log "Successfully started ${job}"
    else
      juju-log "Failed to start ${job}"
      exit ${exitcode}
    fi
  fi
}
...
function stopUpstartJob {
  ...
  then
      juju-log "Job ${job} is already stopped";
  else
    stop ${job} || exitcode=$?
    if [ "0" -eq "${exitcode}" ]
    then
      juju-log "Successfully stopped ${job}"
    else
      juju-log "Failed to stop ${job}"
      exit ${exitcode}
    fi
  fi
}
...
function idempotentInstall {
  ...
  if [[ -z "${package// }" ]];
  then
    juju-log "Could not install. No package name given";
    exit 1;
  fi
  isPackageInstalled ${package} || test=$?
  if [ "0" -ne  "${test}" ]; then
    juju-log "Installing ${package}"
